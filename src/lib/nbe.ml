...
and do_clos2 (D.Clos2 {term; env}) a1 a2 = eval term (D.Term a2 :: D.Term a1 :: env)

and do_clos3 (D.Clos3 {term; env}) a1 a2 a3 = eval term (D.Term a3 :: D.Term a2 :: Term a1 :: env)

and do_open t r =
  match t with
  | D.Shut bclo -> do_bclos bclo r
  | D.Neutral {tp; term} ->
    begin
      match r with
      | D.BVar i ->
...
and eval t (env : D.env) =
  ...
  | Syn.Open (t,r) -> do_open (eval t env) (eval_bdim r env)
...
let rec read_back_nf size nf =
  ...
  | D.Normal {tp = D.Box dest; term} ->
    ...
    let nf = D.Normal {tp = do_bclos dest arg; term = do_open term arg} in
...
and read_back_tp size d =
  ...
  | D.Box dest ->
    ...
    Syn.Box (read_back_tp (size + 1) (do_bclos dest var))
  | D.Id (tp, left, right) ->
    Syn.Id
...
let rec check_nf size nf1 nf2 =
  ...
  | D.Normal {tp = D.Box dest1; term = p1},
    ...
    let arg = D.mk_bvar size in
    let nf1 = D.Normal {tp = do_bclos dest1 arg; term = do_open p1 arg} in
    let nf2 = D.Normal {tp = do_bclos dest2 arg; term = do_open p2 arg} in
    check_nf (size + 1) nf1 nf2
  (* Types *)
...
and check_ne size ne1 ne2 =
  ...
  | D.J (mot1, refl1, tp1, left1, right1, eq1),
    ...
    check_tp ~subtype:false (size + 3) (do_clos3 mot1 mot_var1 mot_var2 mot_var3) (do_clos3 mot2 mot_var1 mot_var2 mot_var3) &&
...
and check_tp ~subtype size d1 d2 =
  ...
  | D.Box dest, D.Box dest' ->
    let var = D.mk_bvar size in
    check_tp ~subtype (size + 1) (do_bclos dest var) (do_bclos dest' var)
 ...
 *     let r = D.BVar (List.length env) in
 *     D.BDim r :: env'
 *   | Syn.Term t :: env ->
 *     let env' = initial_env env in
