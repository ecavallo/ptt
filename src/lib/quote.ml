...
and reduce_extent env size es =
  let rec go env size (e, s) =
    ...
    | D.Undisc :: s -> E.do_undisc (go env size (e, s))
    | D.Extract (_, mot, case) :: s -> E.do_extract size mot (go env size (e, s)) case
    | D.Quasi q :: s ->
      begin
        match q with 
...
and read_back_nf env size nf =
  ...
  (* Discrete *)
  | D.Normal {tp = D.Discrete tp; term = t} ->
    let t' = E.do_undisc t in
    Syn.Endisc (read_back_nf env size (D.Normal {tp; term = t'}))
  (* Types *)
...
and read_back_ne env size (h, s) =
  ...
  | D.Undisc :: s -> Syn.Undisc (read_back_ne env size (h, s))
  | D.Extract (tp, mot, case) :: s ->
    ...
    in
    Syn.Extract (mot', read_back_ne env size (h, s), case')
  | D.Quasi _ :: _ ->
...
and check_ne env size (h1, s1) (h2, s2) =
  ...
  | D.Undisc :: s1, D.Undisc :: s2 -> check_ne env size (h1, s1) (h2, s2)
  | D.Extract (tp1, mot1, case1) :: s1, D.Extract (_, mot2, case2) :: s2 ->
